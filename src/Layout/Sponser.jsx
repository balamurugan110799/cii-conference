...
<h1 className=" font-semibold text-yellow text-h1 pb-4">SPONSORS</h1>
      ...
      {sponser?.map((v,i)=>{
        return(
            <>
            <button  className={` ${v?.status ? "bg-primary" : "bg-yellow "} rounded-lg hover:bg-primary duration-300 py-2 px-8 text-white mr-2 `} onClick={() => handleClick(v?.id)}>{v?.name}</button>
            </>
        )
      })}
              ...
              <div className={` ${v?.status ? "block" : " hidden"}`}>
                <div className=" py-10">
                  <div className=" grid grid-cols-12 gap-6">
                    ...
                    <div className=" col-span-2">
                      <img src={v?.img3} className="h-[100px] object-cover " />
                    </div>
                    <div className=" col-span-2">
                      <img src={v?.img4} className="h-[100px] object-cover " />
                    </div>
                    <div className=" col-span-2">
